...
- Comments: written in English only. Use them when they help the reader — JSDoc with behavior + `@param`/`@returns` on exported functions, hooks, and components; short `//` labels above logical blocks (event handlers, animation steps, conditional branches) as scanning aids, even when slightly redundant with the code; short `//` lines above non-obvious logic (security constraint, business rule, edge case, workaround). Section dividers (`/** --- ROUTES --- */`) are fine to navigate distinct sections in any file. On cleanup, collapse multi-line `/** */` labels into a single-line `//` rather than deleting them — preserve the scanning value. Never restate the symbol name verbatim, never include ticket IDs / commits / caller lists (rots — belongs in PR or commit message), and never stack 3+ `//` lines for one continuous explanation — use a single `/** */` block when multi-line is justified
