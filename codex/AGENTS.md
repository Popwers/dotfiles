...
Delegate to subagents to keep the main context clean. Use one task per subagent with narrow scope and concrete deliverables.

Use Codex roles when available:
- `repo-explorer`: read-only discovery, execution tracing, symbol/config lookup.
- `docs-researcher`: official API/framework documentation checks.
- `planner`: phased implementation plans and architectural decisions.
- `change-implementer`: bounded code changes with explicit file ownership.
- `review-auditor`: bugs, regressions, edge cases, and maintainability risks.
- `security-reviewer`: auth, input, database, shell, filesystem, payment, crypto, and secrets risk.
- `performance-optimizer`: render cost, bundle size, slow queries, and expensive workflows.
- `test-guardian`: coverage gaps, targeted validation, and regression tests.
...
- Comments: written in English only. Add them when they tell the reader something the name and types do not — JSDoc with behavior + `@param`/`@returns` on exported functions, hooks, and components, or a short `//` line above non-obvious logic (security constraint, business rule, edge case, why a pattern is unusual). Section dividers (`/** --- ROUTES --- */`) are fine *inside* a long multi-section file; avoid them at the top of a single-purpose file (the filename already says it). Never restate the symbol name, never include ticket IDs / commits / caller lists (rots — belongs in PR or commit message), and when multi-line is justified use one proper `/** */` block — do not stack `//` lines as a fake multi-line workaround
...
- Default model: `gpt-5.4` with high reasoning for implementation, review, testing, planning, and multi-file reasoning.
- Use `gpt-5.4-mini` for small read-only discovery and low-risk repetitive checks.
- Use `gpt-5.3-codex` for precise bounded terminal-heavy execution when the task is already well specified.
- Prefer CLI tools over MCPs when both achieve the same result with lower context cost.
