...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    try {
      ...
      if (existsSync(gitignore)) {
        ...
        if (!content.split("\n").includes(".grepai/")) {
          writeFileSync(gitignore, content.endsWith("\n") ? content + ".grepai/\n" : content + "\n.grepai/\n");
        }
      }
    ...
    const template = join(process.env.HOME!, ".claude", "claudeignore.template");
  ...
  pi.on("tool_call", async (event, ctx) => {
    if (isToolCallEventType("bash", event)) {
      ...
      const stripped = cmd.replace(/(-m|--message)\s+((?:"[^"]*")|(?:'[^']*')|\S+)/g, "");
      if (/\bgit\s+.*\b(commit|push)\b.*--no-verify|\bgit\s+.*--no-verify\b.*\b(commit|push)\b/.test(stripped)) {
        return {
          block: true,
          reason: "Blocked: --no-verify bypasses husky hooks (tests, biome, commitlint). Remove the flag and fix the underlying issue.",
        };
      }
  ...
  pi.on("tool_result", async (event, ctx) => {
    ...
    if (/\.(ts|tsx|js|jsx|json|css)$/i.test(ext)) {
      ...
      while (dir !== "/") {
        if (existsSync(join(dir, "biome.json")) || existsSync(join(dir, "biome.jsonc"))) {
          try {
            execSync(`bunx @biomejs/biome check --write "${filePath}" --no-errors-on-unmatched`, {
              timeout: 10000,
              cwd: dir,
              stdio: "pipe",
            });
            ...
            ctx.ui.notify(`[Biome] Issues in ${basename(filePath)}: ${tail}`, "error");
          }
          break;
        }
        dir = dirname(dir);
    ...
    // Check for `as any` in TS files
    if (isTS) {
      try {
        const content = readFileSync(filePath, "utf-8");
        if (/\bas\s+any\b/.test(content)) {
          ctx.ui.notify(
            `[Types] \`as any\` found in ${basename(filePath)}. Replace with a type guard or typed helper.`,
            "error"
          );
        }
      } catch {
      ...
      while (dir !== "/") {
        if (existsSync(join(dir, "tsconfig.json"))) {
          try {
            ...
            const errorLines = output.split("\n").filter((l: string) => /error TS/.test(l)).slice(0, 10);
            if (errorLines.length > 0) {
              ctx.ui.notify(
                `[TypeCheck] ${errorLines.length} error(s):\n${errorLines.join("\n")}`,
                "error"
    ...
    if (isFrontend) {
      try {
        ...
        try {
          ...
          if (output.trim()) {
            ctx.ui.notify(`[Impeccable] UI anti-patterns in ${basename(filePath)}:\n${output}`, "error");
          }
        } catch (e: any) {
          const output = (e.stdout?.toString() || "") + (e.stderr?.toString() || "");
          if (output.trim()) {
  ...
  pi.on("agent_end", async (_event, ctx) => {
    ...
    try {
      ...
      const allFiles = [...diffOutput.split("\0"), ...untrackedOutput.split("\0")].filter(Boolean);
    ...
    for (const f of modified) {
      ...
      try {
        execSync(`bun test "${f}"`, { cwd: repoRoot, timeout: 30000, stdio: "pipe" });
      } catch (e: any) {
        const output = (e.stdout?.toString() || "") + (e.stderr?.toString() || "");
        const failures = output.split("\n").filter((l: string) => /FAIL|Error|[✗×]/.test(l)).slice(0, 5);
        issues.push(`[Tests] Failures in ${basename(f)}:\n${failures.join("\n")}`);
      }
    }

    // Biome lint check
    try {
      execSync("command -v biome", { timeout: 3000, stdio: "pipe" });
      try {
        execSync(`biome check ${modified.map((f) => `"${f}"`).join(" ")}`, {
          cwd: repoRoot,
          timeout: 30000,
          stdio: "pipe",
        });
        ...
        const output = (e.stdout?.toString() || "") + (e.stderr?.toString() || "");
        const lines = output.split("\n").filter((l: string) => /error|warning/.test(l)).slice(0, 5);
        if (lines.length > 0) {
    ...
    if (issues.length > 0) {
      ctx.ui.notify(`⚠️ Task quality issues:\n${issues.join("\n\n")}`, "error");
    }
  });
}
...
async function runRelatedTests(filePath: string, ctx: any) {
  ...
  if (testFile && existsSync(testFile)) {
    try {
      ...
      const output = (e.stdout?.toString() || "") + (e.stderr?.toString() || "");
      const failures = output.split("\n").filter((l: string) => /FAIL|Error|[✗×]/.test(l)).slice(0, 10);
      if (failures.length > 0) {
        ctx.ui.notify(`[Tests] Failures in ${basename(testFile!)}:\n${failures.join("\n")}`, "error");
      }
    }
  }
}
